...
SignalK serves SI units (m/s, radians, Kelvin, meters, Pascals); the logbook stores and displays knots, degrees, °C, nautical miles and hectopascals. **All conversion happens in `utils.py`** via the small `_knots` / `_nm` / `_celsius` / `_hpa` / `_signed_deg` / `_bearing_deg` helpers — nothing downstream converts. Wind angles are signed (−180..180, negative = port); headings and courses are normalised 0..360.

Angles and pressure are whole; depth is one decimal. Each is enforced in **four** places, so keep them consistent: the converters in `utils.py`, the explicit `round()` calls in `create_line()`, the identical ones in `update_line()`, and a Jinja filter for display — `deg` for angles, `hpa` for pressure.

**That fourth place is not optional for a whole-number quantity**, because the columns are `REAL`: `round()` hands back an `int`, SQLite stores it as `1030.0`, and it comes back a float. `deg` exists for exactly that reason ("REAL columns hand them back as floats (47.0)"), and pressure needed `hpa` for the same one — `unit('hPa')` printed `1030.0 hPa`. Don't teach `_unit` to round instead: it also serves `nds`, `m`, `NM` and `°C`, which need their decimals. The same float shows up in the **edit** form, where the input prints the stored value directly, so `edit_line.html` formats pressure with `'%.0f'` the way it already formats depth with `'%.1f'`. Input `step` stays `any` there: constraining it to `1` makes the browser *refuse* a pasted decimal instead of letting the handler round it.

**Heading is magnetic, deliberately.** The `heading` endpoint reads `navigation/headingMagnetic` — what the steering compass shows, and what the boat's iKommunicate publishes; it serves no `headingTrue` at all. Deriving true heading from `headingMagnetic + magneticVariation` was tried and **rejected: the result did not match the instruments**. Don't reintroduce it. `cog` remains `courseOverGroundTrue`, so the log's Cap and Route fond are not referenced to the same north — a few degrees apart on this boat.

**Depth has a fallback**, because not every server publishes `environment/depth/belowKeel` — the boat's does not. `_read_depth` tries it first, then `belowTransducer`, which is **not the same measurement**: it is read from the sounder, so it is *larger* than the clearance under the keel. `config.yaml`'s optional `keel_offset` (metres) is subtracted to correct it. Unset it is 0, and the reported depth then overstates the water available — the dangerous direction, which is why `get_keel_offset()` also clamps negatives away.

Pressure comes from `environment/outside/pressure` — `outside`, the ambient reading weather charts use; `inside` is the engine compartment. `SIGNALK_LABELS` in `main.py` names the measurements the connection test reports as *not published*, and its keys must mirror `get_sensor_data()`'s: adding a reading without a label there makes that diagnostic lie by omission, which is what happened to `tws` and `twa`.
...
**Template context** — pass `active_section` (`ship`, `cruises`, `crew`, `routes`, `tools`, `gallery`, `settings`) so `base.html` highlights the right nav item. Each of the six tabs maps to one section, `crew` being Équipiers — its own tab between Navire and Croisières, since crew belong to neither (`crew_members` has no `ship_id`). Six full-size tabs need ~960 px, plus 48 px for the search icon beside the settings gear, which is why the compact-nav media query breaks at 1040 px rather than the 780 it used when there were five. `/search` (magnifier icon) takes `active_section` `None`: it searches across sections through `SEARCH_SECTIONS` in `main.py`, folding case and accents with a `fold()` SQL function registered per connection — add a section there when a new kind of fiche should be findable.
...
**Timestamps** — stored as ISO strings in **local** time via `datetime.now()` (deliberately not UTC — a logbook records boat time). **Display filters** — custom Jinja filters registered at the top of `main.py`, all falling back to an em-dash for missing values: `datefr` (ISO → `DD/MM/YYYY`), `jourfr` (ISO → French weekday, `2026-08-26` → `mercredi`), `age` (birth date → whole years as of today), `euros` (every euro amount in the app, `1100000` → `1.100.000,00 €` — dot for thousands, comma for decimals; empty → `''`, not an em-dash), `deg` (float → whole degrees with the `°` sign, e.g. `47.0` → `47°`), `unit(suffix)` (appends a unit, e.g. `12.4` → `12.4 kn`), `lat` / `lon` (signed decimal degrees → DMM, `43.2891` → `43° 17.346' N`), and `latdmm` / `londmm` (the same split as a dict, for form boxes). Units belong in the *value* via these filters, not in table headers — see the log table in `routes/detail.html`.
